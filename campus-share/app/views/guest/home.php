...
<div class="panel" style="margin-bottom: 30px; padding: 16px 20px; background: #ffffff; border: 1px solid #e2e8f0; border-radius: 8px;">
    <form action="index.php" method="GET" style="display: flex; gap: 12px; align-items: center; width: 100%; flex-wrap: wrap;">
        ...
        <input type="text" name="search" placeholder="Search by title or description..." value="<?= htmlspecialchars($_GET['search'] ?? ''); ?>" style="flex: 2; min-width: 180px; height: 38px; padding: 0 10px; border: 1px solid #cbd5e1; border-radius: 6px;">
...
<!-- Catalog Cards Grid -->
<div id="catalogContainer" style="display: grid; grid-template-columns: repeat(auto-fill, minmax(260px, 1fr)); gap: 20px; margin-bottom: 40px;">
    ...
    <?php else: ?>
        <?php foreach ($resources as $res): ?>
            <div class="resource-card-item"
                 ...
                 data-category="<?= htmlspecialchars($res['category_id']); ?>"
                 data-type="<?= htmlspecialchars($res['sharing_type']); ?>"
                 style="background: #ffffff; border: 1px solid #e2e8f0; border-radius: 8px; padding: 18px; display: flex; flex-direction: column; justify-content: space-between; box-shadow: 0 2px 6px rgba(0,0,0,0.03);">
